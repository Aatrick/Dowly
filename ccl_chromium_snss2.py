...
import types
import typing
from ccl_easy_chromium_pickle import (
    EasyPickleIterator,
    EasyPickleException,
)
...
class SessionRestoreIdType(enum.Enum):
    ...
    CommandTabNavigationPathPrunedFromBack = (
        5  # // OBSOLETE: Superseded by kCommandTabNavigationPathPruned instead
    )
    ...
    CommandTabNavigationPathPrunedFromFront = (
        11  # // Superseded kCommandTabNavigationPathPruned instead
    )
    ...
    CommandSetTabUserAgentOverride = (
        18  # // OBSOLETE: Superseded by kCommandSetTabUserAgentOverride2.
    )
    CommandSessionStorageAssociated = 19
    CommandSetActiveWindow = 20
    CommandLastActiveTime = 21
    CommandSetWindowWorkspace = (
        22  # // OBSOLETE Superseded by kCommandSetWindowWorkspace2.
    )
    CommandSetWindowWorkspace2 = 23
    CommandTabNavigationPathPruned = 24
    CommandSetTabGroup = 25
    CommandSetTabGroupMetadata = (
        26  # // OBSOLETE Superseded by kCommandSetTabGroupMetadata2.
    )
...
class PageTransition:
    ...
    def __init__(self, value):
        ...
        if value < 0:
            # signed to unsigned
            value += 0x80000000 * 2
        self._core_transition = PageTransition._core_transitions[
            value & PageTransition._core_mask
        ]
        self._qualifiers = []
...
class NavigationEntry(SessionCommand):
    ...
    def from_pickle(
        ...
        try:
            ...
            return cls(
                offset, id_type, index, url, title, page_state_raw, transition_type
            )
        ...
        try:
            ...
            if child_task_id_count != 0:
                raise SnssError(
                    "Child tasks should not be present when reading NavigationEntry"
                )
        except EasyPickleException:
            pass
...
class SnssFile:
    def __init__(self, file_type: SnssFileType, stream: typing.BinaryIO):
        ...
        if file_type == SnssFileType.Session:
            ...
            raise ValueError(
                "file_type is an unknown SnssFileType or is not SnssFileType"
            )
    ...
    def _get_next_session_command(self) -> typing.Optional[SessionCommand]:
        ...
        if len(data) != length:
            raise ValueError(
                f"Could not get enough data reading record starting at {start_offset}"
            )
        ...
        # components/sessions/core/tab_restore_service_impl.cc
        if record_id_type in (
            SessionRestoreIdType.CommandUpdateTabNavigation,
            TabRestoreIdType.CommandUpdateTabNavigation,
        ):  # 6, 1
            with EasyPickleIterator(data[1:]) as pickle:
                session_id = pickle.read_int32()
                nav = NavigationEntry.from_pickle(
                    pickle, record_id_type, start_offset, session_id
                )
                return nav
        else:
